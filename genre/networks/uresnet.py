...
from genre.networks.revresnet import revuresnet18, resnet18
import numpy as np
...
class Net_inpaint(nn.Module):
    ...
    def __init__(self, out_planes, layer_names, input_planes=3, out_channels=1):
        super().__init__()

        # Encoder
        module_list = list()
        resnet = resnet18(pretrained=True)
        resnet.bn1.track_running_stats=False
        in_conv = nn.Conv2d(input_planes, 64, kernel_size=7, stride=2, padding=3,
                            bias=False)
        module_list.append(
            nn.Sequential(
        ...
        self.encoder = nn.ModuleList(module_list)
        self.encoder_out = None
        self.deconv2 = nn.ConvTranspose2d(64, out_channels, kernel_size=8, stride=2, padding=3, bias=False, output_padding=0)
        # Decoder
        self.decoders = {}
        for out_plane, layer_name in zip(out_planes, layer_names):
            module_list = list()
            ...
            module_list = nn.ModuleList(module_list)
            setattr(self, 'decoder_' + layer_name, module_list)
            self.decoders = module_list

    def forward(self, im):
        # Encode
        feat = im
        feat_maps = list()
        #print('feat', np.mean(feat.detach().cpu().numpy()))
        for f in self.encoder:
            feat = f(feat)
            #print('feat', np.mean(feat.detach().cpu().numpy()))
            feat_maps.append(feat)
        ...
        outputs = {}
        x = feat_maps[-1]
        for idx, f in enumerate(self.decoders):
            x = f(x)
            #print('x', np.mean(x.detach().cpu().numpy()))
