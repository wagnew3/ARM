...
from genre.loggers.loggers import _LogCumulator
from genre.util.util_print import str_warning
from genre.visualize.visualizer import Visualizer
import pickle
import os
...
class NetInterface(object):
    ...
    def __init__(self, opt, logger):
        ...
        self.grad_hook_gen=None
        self.grad_stats=None
#          = self.dict_grad_hook_factory(
#             add_func=lambda x: {
#                 'mean': x.mean(),
#                 'std': x.std()
#             }
#         )
        ...
        self._moveable_vars = []
        self._optimizers = []
        self.visualizer = None#Visualizer(
#             n_workers=getattr(opt, 'vis_workers', 4),
#             param_f=getattr(opt, 'vis_param_f', None),
#         ) # getattr used for backward compatibility
        self.batches_to_vis = {}
        self.input_names = []
        self._input = Data()#lambda: None
        self.gt_names = []
        self._gt = Data()#lambda: None
        self.aux_names = []
        self._aux = Data()# lambda: None  # auxiliary tensors that need moving to GPU
    ...
    def train_epoch(
        ...
        def _train(epoch):
            ...
            for i in range(steps_per_epoch):
                ...
                print('train start')
                batch_log, batch_loss = self._train_on_batch(epoch, i, data)
                print('train end')
                total_loss+=batch_loss
                print('average train loss:', total_loss/((i+1)*dataloader.batch_sampler.batch_size))
                batch_log['batch'] = i
...
#                 if i>5:
#                     print('exiting in netinterface')
#                     exit()
            print('final average train loss:', total_loss/(steps_per_epoch*dataloader.batch_sampler.batch_size))
        ...
        def _eval(epoch):
            ...
            for i in range(steps_per_eval):
                ...
                data_time = time.time() - start_time
                logger.on_batch_begin(i)
                batch_log, batch_loss = self._vali_on_batch(epoch, i, data)
                total_loss+=batch_loss
                print('average validation loss:', total_loss/((i+1)*dataloader_eval.batch_sampler.batch_size))
                batch_log['batch'] = i
                batch_log['epoch'] = epoch
                batch_log['data_time'] = data_time
                batch_log['batch_time'] = time.time() - start_time
                logger.on_batch_end(i, batch_log)
            print('final average validation loss:', total_loss/(steps_per_eval*dataloader_eval.batch_sampler.batch_size))
            epoch_log = self._internal_logger.get_epoch_log()
            logger.on_epoch_end(epoch, epoch_log)
        ...
        if eval_at_start:
            ...
            with open(os.path.join(save_dir, f'{exp_id}_cds_occlusions.p'), 'wb') as save_file:
                pickle.dump(self.cd_occlusion, save_file) 
            print('exiting netinterface')
            exit()
...
#         torch.cuda.synchronize()
#         print('d', time.time()-s_time)
        a=self._input
        if no_grad:
            with torch.no_grad():
                inputs=(self._input.obs_proj, self._input.obs_voxels)
                pred = net(inputs)  # a structure
