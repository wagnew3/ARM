...
from genre.networks.networks import ViewAsLinear
from genre.networks.uresnet import Net as Uresnet
...
#         torch.cuda.synchronize()
#         print('c', time.time()-s_time)
#         s_time=time.time()
        loss, loss_data = self.compute_loss(pred)
#         torch.cuda.synchronize()
#         print('a', time.time()-s_time)
#         s_time=time.time()
        loss.backward()
        self.optimizer.step()
#         torch.cuda.synchronize()
#         print('b', time.time()-s_time)
        batch_size = len(batch['gt_voxels'])
        batch_log = {'size': batch_size, **loss_data}
        return batch_log, loss.detach().cpu().numpy()*batch_size
