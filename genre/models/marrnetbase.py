...
from genre.models.netinterface import NetInterface
from genre.toolbox.cam_bp.cam_bp.functions import CameraBackProjection
import genre.util.util_img
    ...
    def preprocess(cls, data, mode='train'):
        ...
        for key, val in data.items():
            if key == 'rgb':
                im = val
                # H x W x 3
                im = genre.util.util_img.resize(im, cls.im_size, 'horizontal')
                if mode == 'train':
                    im = genre.util.util_img.jitter_colors(
                        im,
                        d_brightness=cls.rgb_jitter_d,
                        d_contrast=cls.rgb_jitter_d,
                        d_saturation=cls.rgb_jitter_d
                    )
                    im = genre.util.util_img.add_lighting_noise(
                        im, cls.rgb_light_noise)
                im = genre.util.util_img.normalize_colors(im)
                ...
                im = genre.util.util_img.resize(
                    im, cls.im_size, 'horizontal', clamp=(im.min(), im.max()))
                im = genre.util.util_img.binarize(
