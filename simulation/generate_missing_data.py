...
def main():
    ...
    temp = sorted(os.listdir(save_path))
    missing_scenes = []
    for direct in temp:
        blah = os.listdir(save_path + direct)
        if not check_dir(blah):
            missing_scene_num = int(direct.split('_')[1])
            missing_scenes.append(missing_scene_num)
    ...
    for scene_num in missing_scenes:
        printout(f"Working on Scene {scene_num}...")
