    ...
    int inlier_distance) 
{
  ...
  {
    // (cx, cy) is an element in the hough space. this is the index of the 2D hough_space tensor
    int cx = index % width;
    int cy = index / width;
  ...
  CUDA_1D_KERNEL_LOOP(index, nthreads) 
  {
    ...
    if (houghDirectionsPointer[index] > percentageThreshold)
    {
      ...
      if (is_local_max == 1 && (max_index + 1) < max_objects_to_consider)
      {
        ...
        // printf("Max_index: %d, numObjectsPointer: %d", max_index, *numObjectsPointer); // printf can be used on CUDA device. prints to terminal (not jupyter notebook). Note that no stderr is used here. GPU doesn't know what that is
      }
    }
  }
}
    ...
    int* arrays, const int height, const int width, const float inlierThreshold) 
{
  ...
  // int size = *array_size; // TODO: Use this in CUDA_1D_KERNEL_LOOP
  ...
  CUDA_1D_KERNEL_LOOP(index, nthreads) // index is a foreground pixel location
  {
    ...
    if (closest_center_index > -1) // We chose a center
    {
      ...
      // printf("Pixel (%d, %d) chose a center!\n", x, y);
    }
  }
}
    ...
    int object_center_kernel_radius)
{
  ...
  for (int batch_index = 0; batch_index < batch_size; batch_index++)
  {
    ...
    // step 1: compute a label index array
